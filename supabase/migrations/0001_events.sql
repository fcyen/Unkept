...
-- using its server-side insert key (which bypasses RLS). RLS is enabled with
-- NO policies, so the anon key — the only key that could ever reach a browser
...
-- privileged key held solely by the Edge Function can write.
